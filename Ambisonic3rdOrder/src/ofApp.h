...
public:
	...
	inline double angle(const glm::vec3& pV1, const glm::vec3& pV2)
	{
		return std::acos(glm::dot(pV1, pV2) / (glm::length(pV1) * glm::length(pV2)));
	}
};
...
class ofApp : public ofBaseApp{
	...
	public:
		...
		double sample, phaseSineTone, phaseDistance;
		double volume = 0.5;
		double degree, distance;

		ambiDecode2DThirdOrder ambiDec2DThirdOrder;
		ambiEncode2DThirdOrder ambiEnc2DThirdOrder;

		convert3dCoordinate conv3dCoord;
		glm::vec3 vert_pnt_1, vert_pnt_2;
};
